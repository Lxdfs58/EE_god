...
int main( int argc, char* args[] )
{
    try
    {
		...
		while( !quit )
		{
			...
			for(auto &obj : pool)
			{
				if(	(obj->getPosX() < -2*resource::SCREEN_WIDTH ||
					obj->getPosX() > 2*resource::SCREEN_WIDTH ||
					obj->getPosY() < -2*resource::SCREEN_HEIGHT||
					obj->getPosY() > 2*resource::SCREEN_HEIGHT
					||obj->Health<=0)&&obj->GetType()!=ENTITY_TYPE_BARRIER)		
				{
					//pool.remove(obj);
				} 
			}
           ...
           for(auto &obj : pool)
			...
			resource::scoreTexture.render(( resource::SCREEN_WIDTH - resource::gTextTexture.getWidth()-200 ) , 0);
			//Update screen
